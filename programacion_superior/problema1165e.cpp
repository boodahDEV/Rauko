...
/* se invova dentro de buscar_clave_en_personas */
...
int compara_clave_con_personas(const void *a, const void *b)
{
	...
	/* La clave buscada esta en *a sin tipo
	   ...
	   *b sin tipo se hace casting de *b hacia
	   (vida *) y luego apunta a nombre; para comparar 
...
/* Utiliza bsearch para encontrar la clave en la
 estructura personas
*/ 

vida *buscar_clave_en_personas(const void *key,vida personas[MAX], int n)
{
	/* itemptr es el puntero a la estructura vida que
	   almacena la direccion de memoria del primer 
